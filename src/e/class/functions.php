...
function DoReUserHome($add,$userid,$username){
	global $empire,$dbtbpre;
	$ur=$empire->fetch1("select * from {$dbtbpre}enewspage where id='1'");
...
function NewsBq($classid,$indextext,$enews=0,$doing=0){
	...
	elseif($enews==1)//生成首页文件
	{
		...
		//DoReUserHome(); //同步生成指定自定义页 75270060
	}
...
function NotinfoListHtml($path,$list_r,$classlevel){
	...
	//解析支持code标签 75250060
	$pagetext=str_replace("<!--code.start-->","<",$pagetext);
	$pagetext=str_replace("<!--code.end-->",">",$pagetext);
...
function GetHtml($classid,$id,$add,$ecms=0,$doall=0){
	...
	if($pf&&strstr($add[$pf],$expage))//有分页
	{
		...
		for($j=1;$j<=$thispagenum;$j++)
		{
			...
			if($j > $add[vipon]){
			$on=$j-1;
			$string=str_replace('[!--'.$pf.'--]','<div id="vipshow"></div>',$string);
			$string=str_replace('[!--vipshow--]','<script>$.post("/e/extend/vip.php?classid='.$add[classid].'&id='.$add[id].'&page='.$on.'",function(data){$("#vipshow").html(data);});</script>',$string);
			}else{
				$string=str_replace('[!--'.$pf.'--]',$newstext,$string);
				$string=str_replace('[!--vipshow--]','',$string);
			}
...
function ReturnMorepicpathHtml($add){
	...
	$picpath='';
	$path_r=explode("######",$add[morepic]);
	for($pj=0;$pj<count($path_r);$pj++)
    {
		$p=$pj+1;
	    $showdown_r=explode(' ',$path_r[$pj]);
		$picpath.=$showdown_r[0];
	}
	$value=$picpath;
	return $value;
}
...
function ReUserpage($id,$pagetext,$path,$title="",$pagetitle,$pagekeywords,$pagedescription,$tempid=0){
	...
	//解析支持code标签 75250060
	$pagestr=str_replace("<!--code.start-->","<",$pagestr);
	$pagestr=str_replace("<!--code.end-->",">",$pagestr);
...
function ReturnMorepicpath($smallpic,$bigpic,$picname,$delpicid,$picid,$add,$downurl,$down=0,$modtype=0,$fstb=1){
	...
	if(empty($down))
	{
		for($i=0;$i<count($smallpic);$i++)
		{
			...
			if($spic)
			{$returnstr.=$spic.$f_exp.$name.$r_exp;}
		}
	}
	//修改信息
	else
	{
		for($i=0;$i<count($smallpic);$i++)
		{
			...
			if($bpic)
			{$returnstr.=$bpic.$f_exp.$name.$r_exp;}
		}
	}
	//去掉最后的字符
	$returnstr=substr($returnstr,0,strlen($returnstr)-2);
