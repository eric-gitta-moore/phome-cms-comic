...
function BakBuy($userid,$username,$buyname,$userfen,$money,$userdate,$type=0){
	...
	$arr = kou($userid,$money);
	$kou = $arr['kou_money'];
    $fromadd = $arr['fromadd'];
	$empire->query("insert into {$dbtbpre}enewsbuybak(userid,username,card_no,cardfen,money,buytime,userdate,type,kou,fromadd) values('$userid','$username','$buyname','$userfen','$money','$buytime','$userdate','$type','$kou','$fromadd');");
}


//扣量
function  kou($userid,$money){
    global $empire,$dbtbpre;
    $from_sql = $empire->query("select fromadd from {$dbtbpre}enewsmember where userid=$userid");
    $fromadd = '';
    while ($form_r = $empire->fetch($from_sql)) {
        $fromadd =  $form_r[fromadd];
    }
    $kou_money = 0;
    if($fromadd){
        $from_str = '';
        $u_sql = $empire->query("select userid from {$dbtbpre}enewsmember where fromadd='" .$fromadd ."'");
        while ($form_add = $empire->fetch($u_sql)) {
        ...
        if($from_str){
            ...
            $a =  " userid  in(" . $form_str . ")  and buytime<='" . $date . " 23:59:59' and buytime>='" . $date . " 00:00:00';";
        ...
        $k_sql = $empire->query("select `number`,kou,money from {$dbtbpre}enewsingroup where gname='" .$fromadd ."'");
        while ($k_data = $empire->fetch($k_sql)) {
//            $number =$k_data['number']; //分润比例
            $kou = $k_data[kou];//初设扣量比例
            $k_money = $k_data[money];//起扣金额
        }
        if(intval($total) > intval($k_money) && intval($k_money)>0 && intval($kou)>0){
            $kou_ratio = round($zongk/$total,2) * 100;//扣量比例
            if($zongk == 0  ||  intval($kou_ratio) < intval($kou) ){ //今日总扣量/今日总充值 < 初设扣量比例 ? 扣除本次充值金额 :　不扣除
                $kou_money = $money;
            }
        }
    }
    $arr['fromadd'] = $fromadd;
    $arr['kou_money'] = $kou_money;
    return $arr;
}
...
//AJAX检查验证码 75250060
function ecmsCheckShowKeyAjax($varname,$postval,$dopr,$ecms=0,$isadmin=0){
	global $public_r;
	$postval=trim($postval);
	if($isadmin==1)
	{
