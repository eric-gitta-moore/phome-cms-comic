...
<tr bgcolor="#ffffee"><td height="40">作品ID</td><td><input name="zpid" type="text" id="zpid" value="<?=$ecmsfirstpost==1?"":ehtmlspecialchars(stripSlashes($r[zpid]))?>" size="10">
<input type="button" name="changebutton" value="选择" onclick="window.open('db/ChangeField.php?viewf=title&changef=zpid&tbname=comic&form=add&field=zpid<?=$ecms_hashur[ehref]?>','','width=700,height=550,scrollbars=yes');">
<font color="#ff0000">必填</font>&nbsp;&nbsp;&nbsp;&nbsp;第：<input name="num" type="text" id="num" value="<?=$ecmsfirstpost==1?"":ehtmlspecialchars(stripSlashes($r[num]))?>" size="10">
&nbsp;话</td></tr>
<tr bgcolor="#ffffff"><td height="40" width='16%'>标题</td><td><input name="title" type="text" id="title" value="<?=$ecmsfirstpost==1?"":ehtmlspecialchars(stripSlashes($r[title]))?>" size="50">&nbsp;&nbsp;审核：<input name="checked" type="checkbox" value="1"<?=$r[checked]?' checked':''?>>
</td></tr>
<tr bgcolor="#f9f9f9"><td height="40">缩略图</td><td>
...
<tr bgcolor="#ffffff"><td height="40">阅读收费</td><td><input name="price" type="radio" value="1"<?=$r[price]=="1"||$ecmsfirstpost==1?' checked':''?>>免费<input name="price" type="radio" value="2"<?=$r[price]=="2"?' checked':''?>>收费&nbsp;&nbsp;&nbsp;&nbsp;评分：
<input name="score" type="text" id="score" value="<?=$ecmsfirstpost==1?"":ehtmlspecialchars(stripSlashes($r[score]))?>" size="5">
&nbsp;分</td></tr>
...
<tr bgcolor="#ffffff"><td height="40">原文链接</td><td><input name="befrom" type="text" id="befrom" value="<?=$ecmsfirstpost==1?"":ehtmlspecialchars(stripSlashes($r[befrom]))?>" size="50"></td></tr>
</table>
