...
function ReturnUserInfo($userid){
	global $empire,$dbtbpre;
	$userid=(int)$userid;
	$r=$empire->fetch1("select ".eReturnSelectMemberF('username,email,groupid,userfen,money,userdate,zgroupid,checked,registertime,ingid,agid,isern,fromadd')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' limit 1");
	return $r;
}
...
function eAddFenToUser($fen,$date,$groupid,$zgroupid,$user){
	...
	$sql=$empire->query("update ".eReturnMemberTable()." set isern='1',".$update." where ".egetmf('userid')."='".$user[userid]."'");
	if($sql){
		//充值成功返回主页
       printerror("CardGetFenSuccess", "/my/", 1);
	}else{
		//充值不成功
