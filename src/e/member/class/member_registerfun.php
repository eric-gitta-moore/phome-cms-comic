...
function register($add)
{
    ...
    if ($public_r['regkey_ok']) {
        if ($ajax == 1) {
            ecmsCheckShowKeyAjax($keyvname, $add['key'], 1);
        } else {
    ...
    */
	...
	$fromadd = RepPostStr($_POST[fromadd]);
	$edit= $_POST['edit'];
	//注册会员	
    $sql = $empire->query("insert into " . eReturnMemberTable() . "(" . eReturnInsertMemberF('username,password,rnd,email,registertime,groupid,userfen,userdate,money,zgroupid,havemsg,checked,salt,userkey,fromadd,edit') . ") values('{$username}','{$password}','{$rnd}','{$email}','{$registertime}','{$groupid}','{$public_r['reggetfen']}','0','0','0','0','{$checked}','{$salt}','{$userkey}','{$fromadd}','{$edit}');");	
    //取得userid
    ...
    if (!$addr[userid]) {
        $spacestyleid = ReturnGroupSpaceStyleid($groupid);
		 //随机生成头像
        $userpic = $public_r['newsurl'] . 'skin/user/' . rand(1, 100) . '.jpg';
	  	$startpass = $_POST['startpass'];	
        $sql = $empire->query("insert into {$dbtbpre}enewsmemberadd(userid,spacestyleid,regip,lasttime,lastip,loginnum,regipport,lastipport,truename,userpic,startpass) values('{$userid}','{$spacestyleid}','{$regip}','{$lasttime}','{$regip}','1','{$regipport}','{$regipport}','{$username}','{$userpic}','{$startpass}');"); 
		
	   //$sql = $empire->query("insert into {$dbtbpre}enewsmemberadd(userid,spacestyleid,regip,lasttime,lastip,loginnum,regipport,lastipport" . $member_r[0] . ",userpic) values('{$userid}','{$spacestyleid}','{$regip}','{$lasttime}','{$regip}','1','{$regipport}','{$regipport}'" . $member_r[1] . ",'{$userpic}');");
    }
    //更新附件
