...
function CreateClassCache($bclassid,$exp,$expjs,$expmodjs,$adminclass,$doall,$mid,$addminfocid,$oldmid,$oldaddminfocid,$userid){
	...
	while($r=$empire->fetch($sql))
	{
		...
		if($r[islast])
		{
			...
			$addinfo=" <a href='AddNews.php?".$ecms_hashur['ehref']."&enews=AddNews&bclassid=".$r[bclassid]."&classid=".$r[classid]."'>发布</a> / <a href='ListNews.php?".$ecms_hashur['ehref']."&enews=AddNews&bclassid=".$r[bclassid]."&classid=".$r[classid]."'>列表</a>";
		}
		else
	 {
			$img="<img src='../data/images/dir.gif'>";
			$bgcolor="#DBEAF5";
				$divonclick="  id=\"pr".$r[classid]."\" class=\"type".$r[classid]."\" onclick=\"chengstate('".$r[classid]."')\"";
				$start_tbody="<tr id=\"item".$r[classid]."\"><td colspan=\"8\"><table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"1\">
<tbody>";
				$end_tbody="</tbody></table></td></tr>";
				//缩
				$start_tbody1="<tr id=\"item".$r[classid]."\" style=\"display:none\"><td colspan=\"8\"><table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"1\"><tbody>";
			$renewshtml=" <a href='#e' onclick=renews(".$r[classid].",'".$r[tbname]."')>".$fun_r['news']."</a> ";
		}
		//外部栏目
		...
		$onelistclass="<tr bgcolor='".$bgcolor."' height=25><td width='5%'><input type=text name=myorder[] value=".$r[myorder]." size=2><input type=hidden name=classid[] value=".$r[classid]."></td><td".$divonclick."  width='7%'>".$exp.$img."</td><td align=center  width='6%'>".$r[classid]."</td><td width='26%'><input type=checkbox name=reclassid[] value=".$r[classid]."> <a href='".$classurl."' target=_blank>".$classname."</a></td><td align=center width='10%'>".$addinfo."</td><td align=center width='6%'>".$r[onclick]."</td><td width='14%'><a href='#e' onclick=editc(".$r[classid].")>".$fun_r['edit']."</a> <a href='#e' onclick=copyc(".$r[classid].")>".$fun_r['copyclass']."</a> <a href='#e' onclick=delc(".$r[classid].")>".$fun_r['del']."</a></td><td  width='29%'><a href='#e' onclick=relist(".$r[classid].")>".$fun_r['re']."</a>".$renewshtml."<a href='#e' onclick=rejs(".$r[classid].")>JS</a> <a href='#e' onclick=tvurl(".$r[classid].")>调用</a>".$classinfotype.$docinfo."</td></tr>";
